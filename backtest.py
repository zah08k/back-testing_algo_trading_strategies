...
class WalkForward:
    ...
    def wf1(self, firstPar, secondPar, data, train, test, strategy):
        print('wf1 starting')
    ...
    def wf2(self, firstPar, secondPar, data, train, test, strategy):
        print('wf1 starting')
        ...
        for parOne in self.firstPar:
            for parTwo in self.secondPar:
                del score_train[:]

                self.trainResults = self.strategy.strat(self.data, self.train, parOne, parTwo, [], [])
                ...
                if sum(score_train) > best_score:
                    best_firstPar = parOne
                    best_secondPar = parTwo
                    best_score = sum(score_train)

        self.testResults = self.strategy.strat(self.data, self.test, best_firstPar, best_secondPar, [], [])

        print('best par ', best_firstPar, best_secondPar)
        print('One test is done')

        return self.testResults

    def wf1filters(self, firstPar, secondPar, data, train, test, strategy, filters):
