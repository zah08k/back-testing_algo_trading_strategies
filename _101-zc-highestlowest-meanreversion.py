    ...
    def strat(self, data, symbols, parOne, parTwo, filters, filt):
        ...
        for symbol in symbols:
            ...
            for i in self.df.index:
                if self.df.LL[i] and self.df.LC[i] and self.df.FilterLong[i] \
                ...
                # Short
                elif self.df.HH[i] and self.df.HC[i] and self.df.FilterShort[i] \
                        and not inpos and not self.df.Lqdt[i]:
...
def main():
    ...
    directory = 'zc'
    fees = 5.8
    slippage = 0.5 # 2 ticks
    multiplier = 50

    testing_method = 'wf1filters'
    ...
    tr7,te7,tr8,te8,tr9,te9,tr10,te10,tr11,te11,tr12,te12 = traintest.zc()
    ...
    with concurrent.futures.ProcessPoolExecutor() as executor:
        if testing_method == 'wf1':
            ...
            test1 = executor.submit(walkforward.wf1, firstPar, secondPar, data, tr1, te1, Strategy(fees,slippage,multiplier))
            walkforward = WalkForward()
            test2 = executor.submit(walkforward.wf1, firstPar, secondPar, data, tr2, te2, Strategy(fees,slippage,multiplier))
            walkforward = WalkForward()
            test3 = executor.submit(walkforward.wf1, firstPar, secondPar, data, tr3, te3, Strategy(fees,slippage,multiplier))
            walkforward = WalkForward()
            test4 = executor.submit(walkforward.wf1, firstPar, secondPar, data, tr4, te4, Strategy(fees,slippage,multiplier))
            walkforward = WalkForward()
            test5 = executor.submit(walkforward.wf1, firstPar, secondPar, data, tr5, te5, Strategy(fees,slippage,multiplier))
            walkforward = WalkForward()
            test6 = executor.submit(walkforward.wf1, firstPar, secondPar, data, tr6, te6, Strategy(fees,slippage,multiplier))
            walkforward = WalkForward()
            test7 = executor.submit(walkforward.wf1, firstPar, secondPar, data, tr7, te7, Strategy(fees, slippage, multiplier))
            walkforward = WalkForward()
            test8 = executor.submit(walkforward.wf1, firstPar, secondPar, data, tr8, te8, Strategy(fees, slippage, multiplier))
            walkforward = WalkForward()
            test9 = executor.submit(walkforward.wf1, firstPar, secondPar, data, tr9, te9, Strategy(fees, slippage, multiplier))
            walkforward = WalkForward()
            test10 = executor.submit(walkforward.wf1, firstPar, secondPar, data, tr10, te10, Strategy(fees, slippage, multiplier))
            walkforward = WalkForward()
            ...
            walkforward = WalkForward()
            test1 = executor.submit(walkforward.wf1filters, firstPar, secondPar, data, tr1, te1, Strategy(fees,slippage,multiplier), Filters())
            walkforward = WalkForward()
            test2 = executor.submit(walkforward.wf1filters, firstPar, secondPar, data, tr2, te2, Strategy(fees,slippage,multiplier), Filters())
            walkforward = WalkForward()
            test3 = executor.submit(walkforward.wf1filters, firstPar, secondPar, data, tr3, te3, Strategy(fees,slippage,multiplier), Filters())
            walkforward = WalkForward()
            test4 = executor.submit(walkforward.wf1filters, firstPar, secondPar, data, tr4, te4, Strategy(fees,slippage,multiplier), Filters())
            walkforward = WalkForward()
            test5 = executor.submit(walkforward.wf1filters, firstPar, secondPar, data, tr5, te5, Strategy(fees,slippage,multiplier), Filters())
            walkforward = WalkForward()
            test6 = executor.submit(walkforward.wf1filters, firstPar, secondPar, data, tr6, te6, Strategy(fees,slippage,multiplier), Filters())
            walkforward = WalkForward()
            test7 = executor.submit(walkforward.wf1filters, firstPar, secondPar, data, tr7, te7, Strategy(fees, slippage, multiplier), Filters())
            walkforward = WalkForward()
            test8 = executor.submit(walkforward.wf1filters, firstPar, secondPar, data, tr8, te8, Strategy(fees, slippage, multiplier), Filters())
            walkforward = WalkForward()
            test9 = executor.submit(walkforward.wf1filters, firstPar, secondPar, data, tr9, te9, Strategy(fees, slippage, multiplier), Filters())
            walkforward = WalkForward()
            test10 = executor.submit(walkforward.wf1filters, firstPar, secondPar, data, tr10, te10, Strategy(fees, slippage, multiplier), Filters())
            walkforward = WalkForward()
            test11 = executor.submit(walkforward.wf1filters, firstPar, secondPar, data, tr11, te11, Strategy(fees, slippage, multiplier), Filters())
            walkforward = WalkForward()
            test12 = executor.submit(walkforward.wf1filters, firstPar, secondPar, data, tr12, te12, Strategy(fees, slippage, multiplier), Filters())
